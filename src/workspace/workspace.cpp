    ...
    while (true) {
        ...
        switch (c) {
            ...
            case '#': {
                ...
                warning("Shuffle AI is working... (this may take a while)");
                cout << teleport(wherex(), wherey() - 1);
                string text = generateResponse(command);
                cout << teleport(wherex(), wherey() - 1) << erase_line;
                size_t startPos;
                size_t endPos = 0;

                while ((startPos = text.find('`', endPos)) != std::string::npos) {
                    endPos = text.find('`', startPos + 1);
                    if (endPos == string::npos) break;

                    cout << reset << text.substr(0, startPos);

                    cout << bgb_black;
                    cout << text.substr(startPos + 1, endPos - startPos - 1);
                    cout << reset;

                    endPos = endPos + 1;
                    if (size_t nextStartPos = text.find('`', endPos); nextStartPos == string::npos) {
                        cout << reset << text.substr(endPos);
                    }
                    else {
                        cout << reset << text.substr(endPos, nextStartPos - endPos);
                    }
                }

                warning("* This answer was generated by Shuffle AI. It may not be correct.");
                return;
            }
            default: {
